...
class Reverse_IP_Domain:
    ...
    def _pull_domains_socket(cls, ip, verbose=False):
        ...
        try:
            ...
            if verbose:
                console.print(f"[{C6}][-] Socket Exception Error:[{C2}] {e}")
            Variables.errors += 1
    ...
    def _pull_domains_ssl(cls, ip, verbose=False):
        ...
        try:
            ...
            if verbose:
                console.print(f"[{C6}][-] SSL: Timeout connecting to {ip}:443")
            Variables.errors += 1
        except ConnectionRefusedError:
            if verbose:
                console.print(f"[{C6}][-] SSL: Connection refused for {ip}:443")
            Variables.errors += 1
        except Exception as e:
            if verbose:
                console.print(f"[{C6}][-] SSL Exception Error:[{C2}] {e}")
            Variables.errors += 1
    ...
    def _pull_domains_ptr(cls, ip, verbose=False):
        ...
        try:
            ...
            if verbose:
                console.print(f"[{C6}][-] PTR: No PTR record for {ip}")
            Variables.errors += 1
        except dns.resolver.NoAnswer:
            if verbose:
                console.print(f"[{C6}][-] PTR: No answer for {ip}")
            Variables.errors += 1
        except Exception as e:
            if verbose:
                console.print(f"[{C6}][-] PTR Exception Error:[/{C6}] {e}")
            Variables.errors += 1
    ...
    def _threader(cls, max_threads, ips):
        ...
        with ThreadPoolExecutor(max_workers=max_threads) as executor:
            try:
                for ip in ips:
                    futures.append(
                        executor.submit(Reverse_IP_Domain._pull_domains_socket, ip)
                    )
                    futures.append(
                        executor.submit(Reverse_IP_Domain._pull_domains_ssl, ip)
                    )
                    futures.append(
                        executor.submit(Reverse_IP_Domain._pull_domains_ptr, ip)
                    )
    ...
    def _clean_domains(cls, domains):
        ...
        for domain in domains:
            ...
            if (
                len(domain.split(".")[0]) > 30
                and domain.split(".")[0].replace("-", "").isalnum()
            ):
                continue
            ...
            if domain.count(".") == 3 and all(
                part.isdigit() for part in domain.split(".")
            ):
                continue
    ...
    def main(cls):
        ...
        Run.title(
            text="ReverseDNS Results",
            results=len(Variables.found_doms),
